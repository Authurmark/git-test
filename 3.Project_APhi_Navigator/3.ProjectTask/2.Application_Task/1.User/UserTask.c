...
void vUserTask( void *pvParameters )
{
	...
	StrSystemInfo.MAIN_Key = eButtonHoldOff;
	StrSystemInfo.MAIN_Leg_Bike = eButtonHoldOff;
	StrSystemInfo.MAIN_Acce_State = eAcceBikeStand;
	/* Set prequency */
	portTickType xLastWakeTime;
	const portTickType xUser_Task_Frequency = USER_TASK_FREQUENCY;/* 10 tick slice */
	xLastWakeTime = xTaskGetTickCount();
	/* Timer Set */
	timer_set(&pTimerChangeSeflProtect,4,CLOCK_TYPE_S);/* 4s */
	timer_set(&pTimerChangeToLowPower,120,CLOCK_TYPE_S);/* 120s ~ 2minute */
    //timer_set(&pTimerChangeToLowPower,20,CLOCK_TYPE_S);/* 20s */
	timer_set(&pTimerLowPowerStep4Out,15,CLOCK_TYPE_S);/* 15s */
	timer_set(&pEmergencyOut,60,CLOCK_TYPE_S);/* 60s */
	timer_set(&pTimerEnableMode2Leg,300,CLOCK_TYPE_RTC);/* 300s=5' */
    timer_set(&pCheckACCEAlarm,7,CLOCK_TYPE_S);/* 7s */
	...
	{	
        ...
        vIO_ConfigOutput(&OUT_LED_SIGNAL,10,60,6,RELAY_ON,RELAY_OFF,eFALSE);
    }
}
/*********************************************************************/
void vUserTaskMainProcess(void)
